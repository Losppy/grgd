    ...
    client.channels.cache.get(log).send(`${msg.author.tag} Adlı Kullanıcı everyone atmaya çalıştı`)
                 ...
                 client.on('messageCreate', msg => {
                  const filtre = db.fetch(`küfürengel_${msg.guild.id}`)
                     if (filtre) {
                         const kufurler = ["oç", "amk", "ananı sikiyim", "orospu cocu", "orospu cocuğu", "ananıskm", "piç", "amk", "amsk", "sikim", "sikiyim", "orospu çocuğu", "piç kurusu", "kahpe", "orospu", "sik", "yarrak", "amcık", "amık", "yarram", "sikimi ye", "mk", "mq", "aq", "amq",];
                         let kelimeler = msg.content.split(' ');
                         kelimeler.forEach(kelime=> {
